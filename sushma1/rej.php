...
if(isset($_POST['submit']))
{
 ...
 $reason=$_POST['reason'];
	$id=$_GET['rej'];
	$query=mysqli_query($con,"SELECT * from leavetable where LeaveId='".$id."'");
	while($res=mysqli_fetch_array($query,MYSQLI_ASSOC))
 {

	$name=$res['Name']; 
        mysqli_query($con,"INSERT into reason2  values('".$id."','".$reason."')");
	mysqli_query($con,"update leavetable set Status='rejected' where LeaveId='".$id."'");
